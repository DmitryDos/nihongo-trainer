...
export function PracticeMode({
  ...
  // «Знаю» — зеркало «Не знаю»: не вводя ответ, засчитать как верное и показать его.
  function know() {
    if (status !== "idle") return;
    if (dualField) setFieldResult({ reading: true, tr: true });
    resolve(true);
  }

  // «Отклонить» — перезачесть верное как неверное (например, после «Знаю»).
  function reject() {
          ...
          <div className="flex flex-col gap-3">
            ...
            {status === "idle" ? (
              <div className="flex flex-col gap-2">
                ...
                {/* отступ высотой в кнопку */}
                <div className="h-10" aria-hidden />
                <button className="fun-btn fun-primary w-full" onClick={know}>
                  <Check className="size-4" /> Знаю
                </button>
              </div>
            ) : (
              <div className="flex gap-2">
                ...
                {status === "correct" && (
                  <button className="fun-btn fun-neutral" onClick={reject}>
