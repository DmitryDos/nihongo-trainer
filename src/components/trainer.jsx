...
import { Layers, PencilLine, MessageSquare, BookOpen, ListChecks, Boxes, Shuffle, ClipboardCheck } from "lucide-react";
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <Tabs value={tab} onValueChange={setTab} className="flex min-h-0 flex-1 flex-col gap-0">
        ...
        <div className="safe-x min-h-0 flex-1 overflow-x-hidden overflow-y-auto pt-1 pb-3">
          <div className="mx-auto flex w-full max-w-5xl flex-col gap-3">
            ...
            <TabsContent value="reading">
              <ReadingCards filters={filters} presetId={presetId} />
            </TabsContent>
            ...
            <TabsContent value="test">
              <TestMode onAnswered={refreshStats} />
            </TabsContent>
        ...
        {/* Фильтры: часть речи + пресеты — сразу над блоком режимов.
            Только для словарных вкладок; на текстах/предложениях они ни на что не
            влияют при просмотре, поэтому не показываем. */}
        {wordTab && (
          <div className="safe-x shrink-0 pt-1">
            <div className="mx-auto flex w-full max-w-5xl flex-wrap items-center gap-2">
              <MenuSelect
                value={filters.pos}
                onChange={(v) => setFilters({ pos: v })}
                options={[{ value: "all", label: "Все части речи" }, ...POS_OPTIONS]}
                className="w-44"
              />
              <PresetBar presetId={presetId} onChange={setPresetId} />
            </div>
          </div>
        )}
        ...
        <div className="safe-x shrink-0 pt-3 pb-[max(2.25rem,env(safe-area-inset-bottom))]">
          ...
          <TabsTrigger value="test">
            <ClipboardCheck className="size-5" /> Тест
          </TabsTrigger>
